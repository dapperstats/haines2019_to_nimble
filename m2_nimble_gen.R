  ...
  run = function(n   = integer(),
    ...
    for (i in 1:R) {
      ...
      for (j in 1:J_i[i]) {
        ...
        prob[sum(J_i[1:i]) - J_i[i] + i - 1 + j] <- pow(1 - p[i], j - 1) * p[i]
...
registerDistributions(list(
  dM2_nb_vec = list(
    BUGSdist = "dM2_nb_vec(mut, g0, g1, rt, J_i, R, z)",
    Rdist = "dM2_nb_vec(mut, g0, g1, rt, J_i, R, z)",
    discrete = TRUE,
